...
export default function App() {
  ...
  const [homeProfileImage, setHomeProfileImage] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("sangmi_portfolio_home_image");
      if (saved && (saved.includes("Sangmi Kim") || saved.includes("Sangmi%20Kim"))) {
        localStorage.removeItem("sangmi_portfolio_home_image");
        return "https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=800";
      }
      return saved || "https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=800";
    } catch (e) {
      return "https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=800";
    }
  });
  const [aboutProfileImage, setAboutProfileImage] = useState<string>(() => {
    try {
      const saved = localStorage.getItem("sangmi_portfolio_about_image");
      if (saved && (saved.includes("Sangmi Kim") || saved.includes("Sangmi%20Kim"))) {
        localStorage.removeItem("sangmi_portfolio_about_image");
        return "https://images.unsplash.com/photo-1507838153414-b4b713384a76?q=80&w=800";
      }
      return saved || "https://images.unsplash.com/photo-1507838153414-b4b713384a76?q=80&w=800";
    } catch (e) {
      return "https://images.unsplash.com/photo-1507838153414-b4b713384a76?q=80&w=800";
    }
  });
  const [heroBgImage, setHeroBgImage] = useState<string>(() => {
    try {
      ...
      return "https://images.unsplash.com/photo-1552422535-c45813c61732?q=80&w=1600";
    }
  });

  const displayHomeProfileImage = homeProfileImage || "https://images.unsplash.com/photo-1520523839897-bd0b52f945a0?q=80&w=800";
  const displayAboutProfileImage = aboutProfileImage || "https://images.unsplash.com/photo-1507838153414-b4b713384a76?q=80&w=800";
  const displayHeroBgImage = heroBgImage || "https://images.unsplash.com/photo-1552422535-c45813c61732?q=80&w=1600";
  ...
  const [isAdminGateActive, setIsAdminGateActive] = useState(() => {
    try {
      const path = window.location.pathname.toLowerCase();
      const hash = window.location.hash.toLowerCase();
      return path.includes("sangmi-admin-gate") || hash.includes("sangmi-admin-gate");
    } catch (e) {
      return false;
    }
  });
  ...
  const [isAuthorized, setIsAuthorized] = useState(() => {
    try {
      return localStorage.getItem("sangmi_admin_authorized") === "true";
    } catch (e) {
      return false;
    }
  });
  ...
  // Monitor path and hash changes dynamically for sangmi-admin-gate
  useEffect(() => {
    const handleUrlCheck = () => {
      const path = window.location.pathname.toLowerCase();
      const hash = window.location.hash.toLowerCase();
      if (path.includes("sangmi-admin-gate") || hash.includes("sangmi-admin-gate")) {
        setIsAdminGateActive(true);
      }
    };
  ...
  const handleVerifyPassword = (e: React.FormEvent) => {
    e.preventDefault();
    const normalizedPwd = adminPassword.trim();
    if (normalizedPwd === "choven1101") {
      setIsAuthorized(true);
      try {
        localStorage.setItem("sangmi_admin_authorized", "true");
      } catch (e) {}
      setIsAdminGateActive(false);
      setIsCmsOpen(true);
      setPasswordError("");
      setAdminPassword("");
      try {
  ...
  // Load CMS data from server-side persistent database on mount
  useEffect(() => {
    const fetchCmsData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (data) {
            // If server-side database has just been freshly initialized, and this client has existing
            // local edits, we upload the client's current localStorage edits to the server database
            // instead of overwriting them with server defaults.
            const hasLocalEdits = !!(
              localStorage.getItem("sangmi_portfolio_brand") ||
              localStorage.getItem("sangmi_portfolio_home_image") ||
              localStorage.getItem("sangmi_portfolio_about_image") ||
              localStorage.getItem("sangmi_portfolio_hero_bg_image") ||
              localStorage.getItem("sangmi_portfolio_events")
            );

            if (data.isFresh && hasLocalEdits) {
              console.log("Fresh server DB detected. Syncing local edits to server...");
            ...
            if (data.homeProfileImage) setHomeProfileImage(data.homeProfileImage);
            if (data.aboutProfileImage) setAboutProfileImage(data.aboutProfileImage);
            if (data.heroBgImage) setHeroBgImage(data.heroBgImage);
          }
        }
      } catch (err) {
        console.error("Failed to load CMS configurations from server:", err);
      } finally {
        setIsLoadedFromServer(true);
      }
    };
  ...
  // Save changes to server automatically when state updates (debounced)
  useEffect(() => {
    ...
    const saveToServer = async () => {
      try {
        ...
        console.error("Failed to auto-save CMS data to server:", err);
      }
    };

    const debounceId = setTimeout(saveToServer, 800);
    return () => clearTimeout(debounceId);
  }, [brandName, accentColor, events, homeProfileImage, aboutProfileImage, heroBgImage, isLoadedFromServer]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("sangmi_portfolio_home_image", homeProfileImage);
    } catch (e) {
      console.error("Failed to save home profile image:", e);
    }
  }, [homeProfileImage]);

  useEffect(() => {
    try {
      localStorage.setItem("sangmi_portfolio_about_image", aboutProfileImage);
    } catch (e) {
      console.error("Failed to save about profile image:", e);
    }
  }, [aboutProfileImage]);
  ...
  useEffect(() => {
    try {
      ...
      console.error("Failed to save hero bg image:", e);
    }
  }, [heroBgImage]);
  ...
  return (
    <div className="min-h-screen bg-[#08080a] text-slate-200 selection:bg-purple-500 selection:text-white transition-colors duration-300">
      ...
      {/* 2. HERO SECTION */}
      <section id="home" className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden bg-cover bg-center" style={{ backgroundImage: `linear-gradient(to bottom, rgba(8, 8, 10, 0.4) 0%, rgba(8, 8, 10, 0.95) 100%), url('${displayHeroBgImage}')` }}>
      ...
      <CMSAdminPanel
        ...
        setHomeProfileImage={setHomeProfileImage}
        aboutProfileImage={aboutProfileImage}
        setAboutProfileImage={setAboutProfileImage}
        heroBgImage={heroBgImage}
        setHeroBgImage={setHeroBgImage}
      />

      {/* Password Prompt Full-screen Gate for CMS Authentication */}
      {isAdminGateActive && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#08080a] backdrop-blur-xl animate-fade-in p-4 overflow-y-auto">
          {/* Subtle glowing auras */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-brandAccent/10 rounded-full blur-[140px] pointer-events-none" />
          
          <div className="bg-[#0c0c0e] border border-purple-900/40 rounded-3xl w-full max-w-md p-8 relative overflow-hidden shadow-2xl text-left z-10 my-8">
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-purple-600 via-pink-600 to-purple-600 animate-pulse" />
            
            <div className="text-center space-y-3 mb-8 mt-2">
              <div className="inline-flex items-center justify-center w-14 h-14 bg-purple-950/40 border border-purple-800/30 rounded-2xl text-purple-400 mb-2 shadow-lg shadow-purple-950/45">
                <Lock className="w-6 h-6" />
              </div>
              <h3 className="header-serif text-xl font-bold text-white tracking-[0.2em] uppercase">
                {lang === "ko" ? "보안 관리자 게이트" : lang === "zh" ? "安全管理门户" : "Secure Admin Gate"}
              </h3>
              <p className="text-[11px] text-gray-400 leading-relaxed px-4 font-light">
                {lang === "ko" 
                  ? "비공개 관리자 CMS 설정 영역입니다. 계속하려면 액세스 비밀번호를 입력해 주세요." 
                  : lang === "zh"
                    ? "这是专用管理CMS设置区域。请输入访问密码以继续。"
                    : "This is a restricted administrative gateway. Please input your access password to proceed."}
              </p>
            </div>
            ...
            <form onSubmit={handleVerifyPassword} className="space-y-5">
              <div>
                <input
                  ...
                  placeholder={lang === "ko" ? "비밀번호 입력..." : lang === "zh" ? "输入密码..." : "Enter access key..."}
                  className="w-full bg-[#121216] text-white border border-purple-950/60 rounded-xl px-4 py-3 text-xs text-center tracking-widest focus:outline-none focus:border-brandAccent transition shadow-inner font-mono"
                  autoFocus
                />
              </div>

              {passwordError && (
                <p className="text-center text-[11px] text-rose-400 font-semibold tracking-wide animate-pulse">
                  {passwordError}
                </p>
              )}
              ...
              <div className="flex flex-col gap-2 pt-2">
                <button
                  type="submit"
                  className="w-full bg-gradient-to-r from-purple-900 to-indigo-950 hover:from-purple-800 hover:to-indigo-900 border border-purple-800/40 text-white text-xs py-3 rounded-xl transition font-bold shadow-lg shadow-purple-950/30 tracking-widest uppercase cursor-pointer"
                >
                  {lang === "ko" ? "액세스 승인" : lang === "zh" ? "批准访问" : "Authorize Access"}
                </button>
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="w-full bg-transparent hover:bg-white/5 border border-purple-950/50 text-gray-400 hover:text-white text-xs py-3 rounded-xl transition font-semibold tracking-wider uppercase cursor-pointer"
                >
                  {lang === "ko" ? "일반 웹사이트로 돌아가기" : lang === "zh" ? "返回公共网站" : "Return to Public Website"}
                </button>
              </div>
            </form>
          </div>
